...
(async () => {
  ...
  if (env("AI_MODE") === "mock") out(prod ? "FAIL" : "WARN", "AI_MODE=mock returns canned results. Never use it in production");
  else env("ANTHROPIC_API_KEY") ? out("PASS", "ANTHROPIC_API_KEY set (real AI analysis)") : out("WARN", "No ANTHROPIC_API_KEY: certificate analysis is off");
  ...
  env("STRIPE_SECRET_KEY") ? out("PASS", "STRIPE_SECRET_KEY set (billing portal)") : out("WARN", "STRIPE_SECRET_KEY not set: Manage billing button is off");
