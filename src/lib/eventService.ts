...
export async function buildDraft(
 ...
 id = randomUUID();
 const life = parseLifeLimitInput(data);
 Object.assign(data, lifeLimitPayload(life));
 } else {
 ...
 const status = evaluateLifeLimits({
 ...
 return fail(
 409,
 `Life-limited component expired — installation blocked. ${status.reasons.join("; ")}`
 );
 }
 }
...
export async function commitDraft(org: Organization, d: Draft, signature: unknown, expectedPartId?: string) {
 ...
 const life = parseLifeLimitInput(data);
      const part = await tx.part.create({
        data: {
          ...
          currentOrgId: org.id,
          custodyStatus: "QUARANTINE",
          birthCertificateHash:
            typeof data.birthCertificateHash === "string" && data.birthCertificateHash
              ? data.birthCertificateHash
              : d.certificateHash ?? null,
          isLifeLimited: life.isLifeLimited,
          totalTimeHours: life.totalTimeHours,
          totalCycles: life.totalCycles,
          lifeLimitHours: life.lifeLimitHours,
          lifeLimitCycles: life.lifeLimitCycles,
        },
      });
 ...
 data.lifeLimitHours != null ||
 data.lifeLimitCycles != null
 ) {
 const lifeUpdate = parseLifeLimitInput({
 isLifeLimited: part.isLifeLimited || data.isLifeLimited,
 totalTimeHours: data.totalTimeHours ?? part.totalTimeHours,
 totalCycles: data.totalCycles ?? part.totalCycles,
 lifeLimitHours: data.lifeLimitHours ?? part.lifeLimitHours,
 lifeLimitCycles: data.lifeLimitCycles ?? part.lifeLimitCycles,
 });
 await tx.part.update({
 where: { id: part.id },
 data: {
 isLifeLimited: lifeUpdate.isLifeLimited || part.isLifeLimited,
 totalTimeHours: lifeUpdate.totalTimeHours,
 totalCycles: lifeUpdate.totalCycles,
 lifeLimitHours: lifeUpdate.lifeLimitHours,
 lifeLimitCycles: lifeUpdate.lifeLimitCycles,
 },
 });
 }
