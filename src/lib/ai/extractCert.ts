...
/**
 * Resolution order:
 * - Admin sandbox aiModeOverride (in-process)
 * - AI_MODE=off → off
...
export function aiMode(): AiMode {
 const override = getAdminDevFlags().aiModeOverride;
...
export function aiStatus() {
 ...
 const message =
 mode === "anthropic"
 ? "Deterministic OCR pipeline active (cloud vision assist + local flags + AVL / UPN cross-reference)."
 : mode === "local"
 ? raw === "mock"
 ? "Deterministic OCR pipeline (local): PDF text layer → OCR → known-sample fallback (AI_MODE=mock → local)."
 : "Deterministic OCR pipeline (local): PDF text layer → Tesseract OCR → known-sample fallback. Automated FAA UPN cross-reference on extract."
 : "Deterministic OCR pipeline is not configured. Set ANTHROPIC_API_KEY or AI_MODE=local, then restart the server.";
 return { mode, message };
}
