...
import { orgFromApiKey } from "@/lib/auth";
...
export async function POST(req: Request) {
 if (!(await rateLimit(`login:${clientIp(req)}`, 10, 60_000))) return NextResponse.json({ error: "Too many attempts. Wait a minute." }, { status: 429 });
 ...
 if (!org) return NextResponse.json({ error: "API key not recognized" }, { status: 401 });
 let token: string;
